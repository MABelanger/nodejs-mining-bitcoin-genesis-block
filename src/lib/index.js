...
function mineBlock(block){
  ...
  while (!isSolutionFound) {
    ...
    const merkleRoot = sha256Mod.getHexLittleIndian(block.merkleroot);
    ...
    const nonce = sha256Mod.getLittleIndianFromDecimal(block.nonce);               // "1dac2b7c"(integer 2083236893)
    ...
    console.log('__');
    console.log( headerMod.getConcatHeader(version, prevBlockHash, merkleRoot, timestamp, sizeBits, nonce))
